	...
	if($domain_length >210)
	{
		if($i<((scalar @DOMAINS_temp) - 1))
			...
			{	
					 ...
					 &seq($DOMAINS_temp[$i-1],$DOMAINS_ready[0]);
                                	...
                                	$check = 1;
                               		&find_boundary($DOMAINS_temp[$i]);
                               		 if($DOMAINS_ready[0]-$DOMAINS_temp[$i-1] <= 200)
                               		 {
                                       		 print("Adomain ",$i,"starts at \t:",$DOMAINS_temp[$i-1],"\n");
                                       		 print("domain ",$i,"ends at \t:",$DOMAINS_ready[0],"\n");
						 $domain_name = $i;
						 &seq($DOMAINS_temp[$i-1],$DOMAINS_ready[0]);
                               		 }
                              		 else{
                                       		 $modify = ceil(($$DOMAINS_ready[0]-200)/2);
                                      		 print("Bdomain ",$i,"starts at \t:",$DOMAINS_temp[$i-1]+$modify,"\n");
                                       		 print("domain ",$i,"ends at \t:",$DOMAINS_ready[0]-$modify,"\n");
						 $domain_name = $i;
						 &seq($DOMAINS_temp[$i-1]+$modify, $DOMAINS_ready[0]-$modify);
                               		 }
				...
				print("domain ",$i,"(a) starts at  \t:", $first, "\n");
				print("domain ",$i,"(a) ends at  \t:", $new_boundary_end, "\n");
				$domain_name = "$i.a";
				&seq($first,$new_boundary_end);

				if($second - $new_boundary_start <= 200)
				{
					print("domain ",$i,"(b) starts at  \t:", $new_boundary_start, "\n");
					print("domain ",$i,"(b) ends at  \t:", $second, "\n");	
				        $domain_name = "$i.b";	
					&seq($new_boundary_start,$second);
												
				}	
				else{
					$temp = $new_boundary_start;
					&local_maxima($temp, $second);
					print("domain ",$i,"(b) starts at  \t:", $temp, "\n");
					print("domain ",$i,"(b) ends at  \t:", $new_boundary_end, "\n");
					$domain_name = "$i.b";
					&seq($temp,$new_boundary_end);

					if($second - $new_boundary_start <= 200)
					{
						print("domain ",$i,"(c) starts at  \t:", $new_boundary_start, "\n");
						print("domain ",$i,"(c) ends at  \t:", $second, "\n");
						$domain_name = "$i.c";
						&seq($new_boundary_start,$second);
					}
					else{
						print("SORRY BOUNDARIES ARE MUCH LONGER TO SUBDIVIDE"); 
					}
				}
		...
		#	&find_boundary($DOMAINS_temp[$i]); 
			...
			if($second - $new_boundary_start <= 200)
			{
				...
				if($second - $new_boundary_start <= 200)
				{
					...
					print("SORRY BOUNDARIES FOR DOMAIN $i(d) ARE MUCH LONGER TO SUBDIVIDE"); 
				}
			}
