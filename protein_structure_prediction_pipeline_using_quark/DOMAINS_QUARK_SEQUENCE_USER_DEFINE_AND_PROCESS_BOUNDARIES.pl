...
$line_num=0;
print("\n*************************\n");
print("DOMPRED PREDICTION FOR REFERENCE\n"); 
print(@boundary_contents);
print("\n***********************\n\n");


#################################
print("Enter number of domains :"); 
$number_domains = <STDIN>;
chomp($number_domains); 
...
print("Do you wanna process boundaries by yourself or let script handle it? \n");
print("Enter 'Y' for user or 'N' for script (Y/N):  ");
$user_decision = <STDIN>; 
...
if($user_decision =~ 'Y') 
{
	...
	{
        	do{
        		print("Enter boundary ", $i+1," start: ");
			$start = <STDIN>;
        		chomp($start);
			print("Enter boundary ", $i+1," end (maximum is $length): ");
                        $end = <STDIN>;
                        chomp($end);
...
open(input, "$file.diso") || ("could not open a diso file");
@diso = <input>;
chomp(@diso);
#
for($i=0; $i< @diso; $i++)
{
        @diso_split = split(/ +/, $diso[$i]);
	if($diso_split[0] !~ "#"){
                push(@disorder, $diso_split[3]);
	}
}
#################################
...
sub seq {
	...
	$c = 0; 
	print("\n");
	for($start =0; $start<@disorder; $start++)
	{
		 if($start>=$seq_start && $start<=$seq_end)
                {
                        $c++;
                        print($disorder[$start]);
                        if($c == 60)
                        {
                                print "\n";
                                $c = 0;
