...
foreach $file (@domain_input)
{
	...
	do{							#SUBMITS TO SEQATOMS UNTIL SERVER RETURNS THE OUTPUT
		print "SUBMITTING $file TO SEQATOMS\n"; 	
	...
	print "PROCESSING SEQATOMS OUTPUT FILE....\n\n";	
	chomp(@seq_atoms_xml);
	$line_count=0; $i=0;
...
{	
	...
	if($check[$i] == 1) 
	{
		...
		if(grep {$_ eq $i+1} @NO_QUARK)
		{	
			print("\n$domain_name[$i] : USER DOES NOT WANT THIS DOMAIN TO BE SUBMITTED TO QUARK\n");
		}
		else{
		&QUARK($domain_name[$i]);}
	}
...
sub AHEMODEL {
	...
	print "\n$domain_name[$i] : THIS DOMAIN WILL BE SUBMITTED FOR AHEMODEL BUILDING\n";
...
sub QUARK {
	($fasta) = @_; 
	print "\n$domain_name[$i] : THIS DOMAIN WILL BE SUBMITTED TO QUARK\n";
